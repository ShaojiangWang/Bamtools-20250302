...
// swang begin
bool BamMultiReaderPrivate::CreateReaderThreads(const std::vector<std::string>& filenames)
{
    // Launch a thread for each reader
    //std::vector<std::thread> threads; // swang
       m_errorString.clear();
    ...
    for ( int i = 0; filenameIter != filenameEnd; ++filenameIter, i++ ) {
